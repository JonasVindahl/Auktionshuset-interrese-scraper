...
class Store:
    ...
    def record_lot(self, lot: Lot, total: int | None) -> None:
        ...
        with self._tx() as conn:
            row = conn.execute("SELECT last_bid FROM lots WHERE lot_id=?", (lot.lot_id,)).fetchone()
            if row is None:
                conn.execute(
                ...
                if row["last_bid"] != lot.current_bid:
                    log.info("Prisændring på %s: %s -> %s", lot.lot_id, row["last_bid"], lot.current_bid)

            conn.execute(
                """INSERT OR REPLACE INTO price_history (lot_id, observed_at, bid, total)
                   VALUES (?,?,?,?)""",
                (lot.lot_id, now, lot.current_bid, total),
            )
